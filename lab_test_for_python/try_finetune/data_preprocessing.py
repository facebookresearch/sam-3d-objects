...
from util import read_cameras_binary, read_images_binary
import math
class BicycleFinetuneDataset(Dataset):
    def __init__(self, data_dir, split="train"):
        super().__init__()
        self.data_dir = data_dir
        self.image_dir = os.path.join(data_dir, "images_4")
        self.mask_dir = os.path.join(data_dir, "masks")
        self.image_filenames = sorted(os.listdir(self.image_dir))
        self.read_colmap_data(os.path.join(data_dir, "sparse/0"))
...
if __name__ == "__main__":
    ...
    for i, (camera_dict, img_tensor, mask_tensor) in enumerate(dataloader):
        for b in range(img_tensor.shape[0]):
            ...
            print(f"🖼️  影像 Shape: {img_tensor.shape}") # 預期: [1, 3, 822, 1237]
